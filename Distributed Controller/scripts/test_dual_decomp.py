#!/usr/bin/env python3
"""
Test the dual decomposition distributed algorithm.

Sets all desks to occupied with equal costs, enables dual decomposition,
...
CSV_FILE = os.path.join(SCRIPT_DIR, "dual_decomp_results.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Test dual decomposition algorithm")
    parser.add_argument("--port", default=None, help="Serial port (auto-detect if omitted)")
    args = parser.parse_args()

    ser = connect(port=args.port)

    print("\n=== Dual Decomposition Algorithm Test ===\n")

    # Step 1: Set algorithm to dual decomposition
    print("1. Setting algorithm to dual decomposition (A 2)...")
    if not set_algorithm(ser, 2):
    ...
    print(f"3. Waiting for dual decomposition convergence (timeout {CONVERGENCE_TIMEOUT_S}s)...")
    converged = wait_converge(ser, timeout=CONVERGENCE_TIMEOUT_S)
    if converged:
        print("   Dual decomposition converged.")
    else:
